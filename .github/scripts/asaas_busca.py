...
Todos os campos são opcionais e somam: cada um é uma frente de busca própria.
  ...
  CONTA    — "pmm", "wonder" ou vazio pra varrer as duas (o normal).

Roda nas DUAS contas Asaas do grupo. Descartar cobrança viva antes de fechar um
caso de reembolso só vale se as duas foram olhadas: o cliente pode ter contrato
de Wonder e a cobrança viva estar no CNPJ da Wonder.
...
def main():
    ...
    # Conta fora da busca é dito em voz alta: quem lê isto está prestes a concluir
    # "não existe cobrança viva" — e não olhar não é a mesma coisa que não achar.
    ...
    for conta in contas:
        print(f'######## Asaas {conta.rotulo} — cobra {conta.produtos}')
        buscar(conta)


def buscar(conta):
    clientes = asaas(conta, 'customers')
    print(f'{len(clientes)} clientes cadastrados nesta conta.\n')
    ...
    if TERMOS:
        ...
        for c in achados:
            ...
            for p in sorted(asaas(conta, 'payments', customer=c['id']),
